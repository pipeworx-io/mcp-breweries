...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Breweries');
}
...
const tools: McpToolExport['tools'] = [
  {
    ...
    description: 'Search for breweries by name. Returns location, phone, website, and contact details for matching results.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'get_brewery',
    description: 'Fetch full details for a single brewery by its Open Brewery DB UUID. Returns name, type, street address, city, state, postal code, country, GPS coordinates, phone, and website URL.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Find all breweries in a city (e.g., "Portland", "Denver"). Returns location, type, and contact details for each.',
    inputSchema: {
      type: 'object',
      properties: {
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    case 'search_breweries': {
      ...
      if (!res.ok) throw await httpError(res, 'Open Brewery DB error');
    ...
    case 'get_brewery': {
      ...
      const res = await pwFetch(`${BASE_URL}/${encodeURIComponent(id)}`);
      if (!res.ok) {
        if (res.status === 404) throw new Error(`Brewery not found: ${id}`);
        throw await httpError(res, 'Open Brewery DB error');
      }
      const data = (await res.json()) as RawBrewery;
